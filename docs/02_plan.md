...
- seed-777 재증류 = **GPU 하드웨어 사고로 중단** (E110). 31%에서 **PCIe AER 에러(WHEA Event 17)** → GPU lost → 재부팅 복구. 재도전 발사했으나 **자발적 중단**: 기대 +0.0005는 본선컷 미달 확정 상황에서 순위를 못 바꾸는 반면, 14시간 무휴식 풀로드가 실제 하드웨어 위험을 실증했기 때문. **GPU 보호가 우선**
- GPU 정리 완료 (프로세스 종료, 15W 아이들, 54°C)

**⚠️ 교훈**: 장시간 무휴식 GPU 풀로드(>10h)는 PCIe/전력 계통 하드웨어 위험. 향후 실험 체인에 냉각 간격 필수.
...
4. ✅ **R-Drop Qwen3 fold0 = 0.7636 기각** (02:24 완주, 곡선 0.7190→0.7611→0.7636). 게이트 −0.0063, XLM-R 레버 Qwen3 전이 3연속 실패. 애초에 시간 물리로도 활용 불가였음(기록용)
4b. ⏳ **seed-777 재증류 (밤샘 체인, 02:25 발사~07:00)** — 잔여 GPU 마지막 +EV = LB 재추첨(±0.001~2). max(au07, s777) 선택, private 기대값 동일이라 편향 무해
...
**제출 파일 위치**: da2 루트 — **`submit_distill_au07.zip`=현 최고 0.77374** / `submit_s777_au07.zip`=밤샘 체인 생성 예정 / distill3w·au05·au13·au03·full3ep·2way=기록용

## 목표와 현실

- 목표: **LB 0.80** (07-12 사용자 상향. 참고선: 본선권 12위 0.79307, 1위 0.79795).
- 현재: **LB 0.77374** (E108 distill3w+au×0.7, 07-14 밤). 마감: **7/15 10:00**. 07-14 제출 5회 사용.
- **최종 결산 (07-14 밤)**: 07-13 예측한 "잔여 카드 합산 +0.003~0.006 (0.774~0.777)"이 실측과 정합 — 0.7677→**0.77374** (+0.0060) 달성. 경로: full-data(+0.0035)+2way증류(+0.0014)+3way교사(+0.0006)+au강도(+0.0005). 학습 레버(1.7B·Embedding·lr·group CE·BSM)는 전부 기각, **모델 확대·표현 강화 무효 패턴 최종 확인**. R107 진단으로 천장 규명(초기 nav 정보한계). **목표 0.80·본선컷 0.793 미달 확정.** 남은 카드 = seed 재추첨(E110)뿐.
