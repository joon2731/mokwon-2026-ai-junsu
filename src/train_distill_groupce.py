# -*- coding: utf-8 -*-
"""3-way 교사 증류 + 혼동집합 조건부 CE 결합, 0.6B 학생 full-data 학습.

E107 full-data 경로 (fold0 게이트 통과 시). 현 최고 = E105 distill3w(LB 0.77326)의
증류 베이스 위에 group CE 보조손실을 얹어 그룹 내부 경계를 선명화한다.

  L = α·CE_sqrt + (1−α)·T²·KL(student/T || teacher/T) + λ·CE_group
    T=3, α=0.6 (E105와 동일), λ=group_ce_lambda
...
def get_args():
    ...
    p.add_argument("--tag", default="qwen3_distill3w_groupce")
    return p.parse_args()
...
class DistillGroupCETrainer(Trainer):
    def __init__(self, class_weights=None, temperature=3.0, alpha=0.6,
                 group_ids=None, group_ce_lambda=0.0, **kw):
        super().__init__(**kw)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.group_ce_lambda > 0:
            ...
            sample_group = gid[labels]
            mask = gid.unsqueeze(0) == sample_group.unsqueeze(1)
            masked = logits.masked_fill(~mask, float("-inf"))
            logp = torch.log_softmax(masked, dim=-1)
            nll = -logp.gather(1, labels.unsqueeze(1)).squeeze(1)  # singleton-group -> 0
            if w is not None:
                sw = w[labels]
                ce_group = (nll * sw).sum() / sw.sum().clamp_min(1e-8)
            else:
                ce_group = nll.mean()
            loss = loss + self.group_ce_lambda * ce_group
...
def main():
    ...
    print(f"DISTILL+GROUPCE full-data: n={len(df)} T={args.temperature} alpha={args.alpha} "
          f"lambda={args.group_ce_lambda} teacher={os.path.basename(args.teacher_npz)} "
    ...
    trainer = DistillGroupCETrainer(
        class_weights=cw, temperature=args.temperature, alpha=args.alpha,
        group_ids=group_ids, group_ce_lambda=args.group_ce_lambda,
